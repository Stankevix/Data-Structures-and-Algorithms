...
void insereTabela(Hashtable *tabela, Chave chave);
...
int insereTabela(Hashtable *tabela, Chave chave){
	...
	if (tabela->v[h].estado == LIVRE){
		strcpy(tabela->v[h].pk,chave.pk);
		tabela->v[h].rrn = chave.rrn;
		...
		if (tabela->v[h].estado == OCUPADO){
			...
			for (i = h+1; i < tabela->tam; i++){
				if(tabela->v[i].estado == LIVRE){
					strcpy(tabela->v[i].pk,chave.pk);
					tabela->v[i].rrn = chave.rrn;
...
void cadastrar(Hashtable *tabela){
	Jogador jogador;
	Chave chave;
	int flagPartidas=0, flagData=0, i = 0, flagRepetido = 0, posicao_relativa = 0,maior = 0,colisao = 0;
	...
	if ( flagRepetido == (-1)){
		...
		maior = tabela->v[0].rrn;
		for (i = 0; i< tabela->tam; i++){
			if (tabela->v[i].estado == OCUPADO){
				if(tabela->v[i].rrn > maior){
					maior = tabela->v[i].rrn;
				}
			}
		}

	    chave.rrn = maior + TAM_REGISTRO;	
		colisao = insereTabela(tabela, chave);
		printf(REGISTRO_INSERIDO, colisao);
